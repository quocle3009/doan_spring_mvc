					...
					<div id="navbar" class="navbar-collapse collapse">
						<ul class="nav navbar-nav">
							...
							<li><a href="<c:url value="/" />"><spring:message
										code="home.page" /></a></li>
							<li><a
								href="<c:url value="/product/productList/Laptop/1" />"><spring:message
										code="product.page" /></a></li>
							<li><a href="<c:url value="/about" />"><spring:message
										code="about.page" /></a></li>
							<li><a href="<c:url value="/contact" />"><spring:message
										code="contact.page" /></a></li>
						</ul>
						<ul class="nav navbar-nav pull-right">
							<c:if test="${pageContext.request.userPrincipal.name!=null}">
								<li><a>Welcome:${pageContext.request.userPrincipal.name}
								</a></li>
								<c:if test="${pageContext.request.userPrincipal.name!='admin'}">
									<li role="presentation" class="dropdown"><a
										...
										<ul class="dropdown-menu">
											<li><a href="<c:url value="/customer/update"/>"><spring:message
														code="updateAccount.page" /></a></li>
										</ul></li>

								</c:if>
								<li><a href="<c:url value="/logout" />"><spring:message
											code="logout.page" /></a></li>
								<c:if test="${pageContext.request.userPrincipal.name!='admin'}">

									<li><a href="<c:url value="/customer/cart"/>"><spring:message
												code="cart.page" /></a></li>
								</c:if>
								<c:if test="${pageContext.request.userPrincipal.name =='admin'}">
									<li><a href="<c:url value="/admin"/>"><spring:message
												code="admin.page" /></a></li>
								</c:if>
							</c:if>
							<c:if test="${pageContext.request.userPrincipal.name == null}">
								<li><a href="<c:url  value="/login"/>"><spring:message
											code="login.page" /></a></li>
								<li><a href="<c:url  value="/register"/>"><spring:message
											code="register.page" /></a></li>
							</c:if>
						</ul>
					</div>
				</div>
